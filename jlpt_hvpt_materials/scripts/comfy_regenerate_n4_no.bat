...
set "REGISTER_SCRIPT=D:\codex-2\jlpt_hvpt_materials\scripts\register_n4_tup_review.py"
set "GEN_SCRIPT=D:\codex-2\jlpt_hvpt_materials\scripts\comfy_batch_generate.py"
set "OUTDIR=D:\codex-2\tup_n4"

echo Registering current N4 review folders before retry...
"%PYTHON%" "%REGISTER_SCRIPT%"
if errorlevel 1 (
  echo.
  echo FAIL: N4 review registration failed.
  pause
  exit /b 1
)
...
echo.
echo Regenerating only N4 rows marked NO.
