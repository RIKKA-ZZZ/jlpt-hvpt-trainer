...
def backup_input(path: Path) -> Path:
    ...
    backup = path.with_name(f"{path.stem}.before_n4_no_picture_{stamp}{path.suffix}")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Mark N4 words that should not be sent to image generation as NO PICTURE."
    )
    ...
    parser.add_argument(
        "--overwrite-existing",
        action="store_true",
        help="Allow replacing existing N4 review markers. By default, OK/NO/NO PICTURE are preserved.",
    )
    ...
    for row in rows:
        ...
        if row.get("jlpt_level") == "N4" and row.get("image_review", "").strip() != NO_PICTURE:
            candidates.append(
                {
                    "id": row.get("id", ""),
                    "word": row.get("word", ""),
    ...
    summary = {
        ...
        "n4_total": sum(1 for row in rows if row.get("jlpt_level") == "N4"),
        "preserved_existing_review": preserved,
        "new_no_picture_marks": len(marked),
        "n4_generation_candidates": len(candidates),
    }

    DERIVED.mkdir(parents=True, exist_ok=True)
    write_csv(
        DERIVED / "n4_no_picture_marked_rows.csv",
        marked,
    ...
    (DERIVED / "n4_no_picture_summary.json").write_text(
