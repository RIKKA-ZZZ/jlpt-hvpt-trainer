...
EXTRA_NO_PICTURE_IDS = {
    "n2_03679",  # サークル: circle / club is too broad and easy to misread
    "n2_03819",  # 乗車: boarding/taking a train is an action, not a stable object
    "n2_03890",  # 保健: health preservation / hygiene is abstract
    "n2_03917",  # 先頭: lead / foremost, not a body-part "head"
    "n2_04063",  # 各地: every place / various places
    "n2_04078",  # 向う: to face / go toward was classified as a body term
    "n2_04125",  # 在学: enrollment status
    "n2_04127",  # 地名: place name
    "n2_04204",  # 学年: school year / grade
    "n2_04481",  # 断水: water outage
    "n2_04772",  # 番地: house number / address
    "n2_04794",  # 目次: table of contents relies on text
    "n2_04811",  # 真っ先: foremost / beginning
    "n2_04814",  # 真っ青: color/state
    "n2_04815",  # 真っ黒: color/state
    "n2_04859",  # 空中: in the air / sky is too unspecific
    "n2_05006",  # 衣食住: necessities of life
    "n2_05119",  # 退院: leaving hospital
    "n2_05222",  # 集金: collecting money is an action/event
    "n2_05248",  # 頭脳: brains / intellect
}
...
def no_picture_reason(row: dict[str, str]) -> str:
    if row.get("id") in EXTRA_NO_PICTURE_IDS:
        return "N2 manual filter: abstract, relational, or easily misread"
...
def backup_input(path: Path) -> Path:
    ...
    backup = path.with_name(f"{path.stem}.before_n2_no_picture_{stamp}{path.suffix}")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Mark N2 words that should not be sent to image generation as NO PICTURE."
    )
    ...
    parser.add_argument(
        "--overwrite-existing",
        action="store_true",
        help="Allow replacing existing N2 review markers. By default, OK/NO/NO PICTURE are preserved.",
    )
    ...
    for row in rows:
        ...
        if row.get("jlpt_level") == "N2" and row.get("image_review", "").strip() != NO_PICTURE:
            candidates.append(
                {
                    "id": row.get("id", ""),
                    "word": row.get("word", ""),
    ...
    summary = {
        ...
        "n2_total": sum(1 for row in rows if row.get("jlpt_level") == "N2"),
        "preserved_existing_review": preserved,
        "new_no_picture_marks": len(marked),
        "n2_generation_candidates": len(candidates),
    }

    DERIVED.mkdir(parents=True, exist_ok=True)
    write_csv(
        DERIVED / "n2_no_picture_marked_rows.csv",
        marked,
        ["id", "word", "reading", "meaning", "image_type", "visual_domain", "image_query_en", "reason"],
    )
    write_csv(
        DERIVED / "n2_image_generation_candidates.csv",
        candidates,
        ["id", "word", "reading", "meaning", "image_type", "visual_domain", "image_query_en", "review"],
    )
    (DERIVED / "n2_no_picture_summary.json").write_text(
