...
DEFAULT_TUP_FILES = ROOT / "tup_n3" / "files"
...
def backup_input(path: Path) -> Path:
    ...
    backup = path.with_name(f"{path.stem}.before_n3_tup_review_{stamp}{path.suffix}")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Register N3 generated image review results from tup_n3/files and rename files with Chinese meanings."
    )
    ...
    write_csv(
        DERIVED / "n3_tup_review_registered_rows.csv",
        current_rows,
    ...
    write_csv(
        DERIVED / "n3_current_review_rows.csv",
        n3_review_rows,
        [
            "id",
            "word",
    ...
    (DERIVED / "n3_tup_review_summary.json").write_text(
