...
set "MARK_SCRIPT=D:\codex-2\jlpt_hvpt_materials\scripts\mark_n4_no_picture.py"
set "GEN_SCRIPT=D:\codex-2\jlpt_hvpt_materials\scripts\comfy_batch_generate.py"
set "OUTDIR=D:\codex-2\tup_n4"

echo Preparing N4 vocabulary review markers...
"%PYTHON%" "%MARK_SCRIPT%"
if errorlevel 1 (
  echo.
  echo FAIL: N4 prefilter failed.
  pause
  exit /b 1
)
...
echo.
echo Generating N4 imageable vocabulary images.
