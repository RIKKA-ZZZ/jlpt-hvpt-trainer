...
set "MARK_SCRIPT=D:\codex-2\jlpt_hvpt_materials\scripts\mark_n1_no_picture.py"
set "GEN_SCRIPT=D:\codex-2\jlpt_hvpt_materials\scripts\comfy_batch_generate.py"
set "OUTDIR=D:\codex-2\tup_n1"

echo Preparing N1 vocabulary review markers...
"%PYTHON%" "%MARK_SCRIPT%"
if errorlevel 1 (
  echo.
  echo FAIL: N1 prefilter failed.
  pause
  exit /b 1
)
...
echo.
echo Generating N1 imageable vocabulary images.
