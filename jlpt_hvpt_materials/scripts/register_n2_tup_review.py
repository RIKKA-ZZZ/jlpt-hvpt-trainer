...
DEFAULT_TUP_FILES = ROOT / "tup_n2" / "files"
...
def desired_name(path: Path, row: dict[str, str]) -> str:
    ...
    if meaning_zh:
        ...
        parts.append(safe_name(compact_english_meaning(row.get("meaning", "")) or "meaning"))
...
def backup_input(path: Path) -> Path:
    ...
    backup = path.with_name(f"{path.stem}.before_n2_tup_review_{stamp}{path.suffix}")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Register N2 generated image review results from tup_n2/files and rename files with Chinese meanings."
    )
    ...
    write_csv(
        DERIVED / "n2_tup_review_registered_rows.csv",
        current_rows,
    ...
    write_csv(
        DERIVED / "n2_current_review_rows.csv",
        n2_review_rows,
        [
            "id",
            "word",
    ...
    (DERIVED / "n2_tup_review_summary.json").write_text(
