...
set "MARK_SCRIPT=D:\codex-2\jlpt_hvpt_materials\scripts\mark_n3_no_picture.py"
set "GEN_SCRIPT=D:\codex-2\jlpt_hvpt_materials\scripts\comfy_batch_generate.py"
set "OUTDIR=D:\codex-2\tup_n3"

echo Preparing N3 vocabulary review markers...
"%PYTHON%" "%MARK_SCRIPT%"
if errorlevel 1 (
  echo.
  echo FAIL: N3 prefilter failed.
  pause
  exit /b 1
)
...
echo.
echo Generating N3 imageable vocabulary images.
